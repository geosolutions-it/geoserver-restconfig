...
try:
    ...
    conn = psycopg2.connect(
        (
            "dbname=%(database)s user=%(user)s password=%(passwd)s"
...
if GSPARAMS["GEOSERVER_HOME"]:
    ...
    os.system(
        f"curl -XPOST --user '{GSPARAMS['GSUSER']}':'{GSPARAMS['GSPASSWORD']}' '{GSPARAMS['GSURL']}/reload'"
    )
...
if GSPARAMS["GS_VERSION"]:
    ...
    proc = subprocess.Popen(
        [
            ...
            "-jar",
            f"{GSPARAMS['GS_BASE_DIR']}/gs/jetty-runner-9.4.5.v20170502.jar",
            "--path",
            "/geoserver",
            f"{GSPARAMS['GS_BASE_DIR']}/gs/geoserver.war",
        ],
        stdout=FNULL,
...
class ServicesTests(unittest.TestCase):
    def setUp(self):
        ...
        self.bkp_wms_srv.dirty = {
            k: getattr(self.bkp_wms_srv, k) for k, v in self.bkp_wms_srv.writers.items()
        }
        self.bkp_wfs_srv = self.bkp_cat.get_services(ogc_type="wfs")[0]
        self.bkp_wfs_srv.dirty = {
            k: getattr(self.bkp_wfs_srv, k) for k, v in self.bkp_wfs_srv.writers.items()
        }
        self.bkp_wcs_srv = self.bkp_cat.get_services(ogc_type="wcs")[0]
        self.bkp_wcs_srv.dirty = {
            k: getattr(self.bkp_wcs_srv, k) for k, v in self.bkp_wcs_srv.writers.items()
        }
        self.bkp_wmts_srv = self.bkp_cat.get_services(ogc_type="wmts")[0]
        self.bkp_wmts_srv.dirty = {
            k: getattr(self.bkp_wmts_srv, k)
            for k, v in self.bkp_wmts_srv.writers.items()
        }

        # test workspace
        self.test_ws = self.cat.get_workspace("services_test_ws")
        if self.test_ws is None:
            self.test_ws = self.cat.create_workspace(
                "services_test_ws", uri=self.cat.service_url
            )

        # enums for tests
        self.wms_enums = {"interpolation": ["Nearest", "Bilinear", "Bicubic"]}
        self.wfs_enums = {"serviceLevel": ["BASIC", "TRANSACTIONAL", "COMPLETE"]}
        self.wcs_enums = {}
        self.wmts_enums = {}
    ...
    def test_global_wms(self):
        ...
        attrs = [
            k
            for k, v in wms_srv.writers.items()
            if isinstance(getattr(wms_srv, k), bool)
        ]
        for attr in attrs:
            ...
            wms_srv.refresh()
            self.assertIsNone(
                wms_srv.dirty.get(attr), msg=f"Attribute {attr} still in dirty list"
            )
            ...
            self.assertTrue(
                getattr(wms_srv, attr), msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k, v in wms_srv.writers.items()
            if isinstance(getattr(wms_srv, k), str) and k not in self.wms_enums.keys()
        ]
        for attr in attrs:
            ...
            self.assertEqual(
                getattr(wms_srv, attr), test_str, msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k in wms_srv.writers.keys()
            if isinstance(getattr(wms_srv, k), int)
            and not isinstance(getattr(wms_srv, k), bool)
        ]
        for attr in attrs:
            ...
            self.assertEqual(
                getattr(wms_srv, attr), test_int, msg=f"Invalid value for object {attr}"
            )
    ...
    def test_global_wfs(self):
        ...
        attrs = [
            k
            for k, v in wfs_srv.writers.items()
            if isinstance(getattr(wfs_srv, k), bool)
        ]
        for attr in attrs:
            ...
            wfs_srv.refresh()
            self.assertIsNone(
                wfs_srv.dirty.get(attr), msg=f"Attribute {attr} still in dirty list"
            )
            ...
            self.assertTrue(
                getattr(wfs_srv, attr), msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k, v in wfs_srv.writers.items()
            if isinstance(getattr(wfs_srv, k), str) and k not in self.wfs_enums.keys()
        ]
        for attr in attrs:
            test_str = "".join(random.sample(string.ascii_lowercase, 10))
            ...
            self.assertEqual(
                getattr(wfs_srv, attr), test_str, msg=f"Invalid value for object {attr}"
            )

        # test enums
        attrs = [k for k in self.wfs_enums.keys()]
        for attr in attrs:
            test_str = self.wfs_enums[attr][
                random.randint(0, len(self.wfs_enums[attr]))
            ]
            setattr(wfs_srv, attr, test_str)
            self.cat.save(wfs_srv)
            wfs_srv.refresh()
        ...
        attrs = [
            k
            for k in wfs_srv.writers.keys()
            if isinstance(getattr(wfs_srv, k), int)
            and not isinstance(getattr(wfs_srv, k), bool)
        ]
        for attr in attrs:
            test_int = random.randint(1, 20)
            ...
            self.assertEqual(
                getattr(wfs_srv, attr), test_int, msg=f"Invalid value for object {attr}"
            )
    ...
    def test_global_wcs(self):
        ...
        attrs = [
            k
            for k, v in wcs_srv.writers.items()
            if isinstance(getattr(wcs_srv, k), bool)
        ]
        for attr in attrs:
            ...
            wcs_srv.refresh()
            self.assertIsNone(
                wcs_srv.dirty.get(attr), msg=f"Attribute {attr} still in dirty list"
            )
            ...
            self.assertTrue(
                getattr(wcs_srv, attr), msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k, v in wcs_srv.writers.items()
            if isinstance(getattr(wcs_srv, k), str) and k not in self.wcs_enums.keys()
        ]
        for attr in attrs:
            test_str = "".join(random.sample(string.ascii_lowercase, 10))
            ...
            self.assertEqual(
                getattr(wcs_srv, attr), test_str, msg=f"Invalid value for object {attr}"
            )

        # test enums
        attrs = [k for k in self.wcs_enums.keys()]
        for attr in attrs:
            test_str = self.wcs_enums[attr][
                random.randint(0, len(self.wcs_enums[attr]))
            ]
            setattr(wcs_srv, attr, test_str)
            self.cat.save(wcs_srv)
            wcs_srv.refresh()
        ...
        attrs = [
            k
            for k in wcs_srv.writers.keys()
            if isinstance(getattr(wcs_srv, k), int)
            and not isinstance(getattr(wcs_srv, k), bool)
        ]
        for attr in attrs:
            test_int = random.randint(1, 20)
            ...
            self.assertEqual(
                getattr(wcs_srv, attr), test_int, msg=f"Invalid value for object {attr}"
            )
    ...
    def test_global_wmts(self):
        ...
        attrs = [
            k
            for k, v in wmts_srv.writers.items()
            if isinstance(getattr(wmts_srv, k), bool)
        ]
        for attr in attrs:
            ...
            wmts_srv.refresh()
            self.assertIsNone(
                wmts_srv.dirty.get(attr), msg=f"Attribute {attr} still in dirty list"
            )
            ...
            self.assertTrue(
                getattr(wmts_srv, attr), msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k, v in wmts_srv.writers.items()
            if isinstance(getattr(wmts_srv, k), str) and k not in self.wmts_enums.keys()
        ]
        for attr in attrs:
            test_str = "".join(random.sample(string.ascii_lowercase, 10))
        ...
        for attr in attrs:
            test_str = self.wmts_enums[attr][
                random.randint(0, len(self.wmts_enums[attr]))
            ]
            setattr(wmts_srv, attr, test_str)
            self.cat.save(wmts_srv)
            wmts_srv.refresh()
        ...
        attrs = [
            k
            for k in wmts_srv.writers.keys()
            if isinstance(getattr(wmts_srv, k), int)
            and not isinstance(getattr(wmts_srv, k), bool)
        ]
        for attr in attrs:
            test_int = random.randint(1, 20)
...
if __name__ == "__main__":
    ans = input(
        "This test could break your Geoserver configuration - are U sure? (y/n)"
    )
    if ans.lower() == "y":
        unittest.main()
    else:
