...
class SettingsTests(unittest.TestCase):
    ...
    def test_set_global(self):
        ...
        attrs = [
            k
            for k, v in test_class.writers.items()
            if isinstance(getattr(test_class, k), bool)
        ]
        ...
        for attr in attrs:
            ...
            test_class.refresh()
            self.assertIsNone(
                test_class.dirty.get(attr), msg=f"Attribute {attr} still in dirty list"
            )
            ...
            self.assertTrue(
                getattr(test_class, attr), msg=f"Invalid value for object {attr}"
            )
        ...
        attrs = [
            k
            for k, v in test_class.writers.items()
            if isinstance(getattr(test_class, k), str) and k not in enums.keys()
        ]
        for attr in attrs:
        ...
        attrs = [
            k
            for k in test_class.writers.keys()
            if isinstance(getattr(test_class, k), int)
            and not isinstance(getattr(test_class, k), bool)
        ]
        for attr in attrs:
    ...
    def test_set_settings(self):
        ...
        subclasses = [
            x
            for x in glob.writers.keys()
            if issubclass(type(getattr(glob, x)), StaticResourceInfo)
        ]
        ...
        for subcls in subclasses:
            ...
            attrs = [
                k
                for k, v in getattr(glob, subcls).writers.items()
                if isinstance(getattr(getattr(glob, subcls), k), bool)
            ]
            ...
            for attr in attrs:
                ...
                glob.refresh()
                self.assertIsNone(
                    getattr(glob, subcls).dirty.get(attr),
                    msg=f"Attribute {attr} still in dirty list",
                )
                ...
                self.assertTrue(
                    getattr(getattr(glob, subcls), attr),
                    msg=f"Invalid value for object {attr}",
                )
            ...
            attrs = [
                k
                for k, v in getattr(glob, subcls).writers.items()
                if isinstance(getattr(getattr(glob, subcls), k), str)
                and k not in enums.keys()
            ]
            for attr in attrs:
                test_str = "".join(random.sample(string.ascii_lowercase, 10))
            ...
            attrs = [
                k
                for k in getattr(glob, subcls).writers.keys()
                if isinstance(getattr(getattr(glob, subcls), k), int)
                and not isinstance(getattr(getattr(glob, subcls), k), bool)
            ]
            for attr in attrs:
                test_int = random.randint(1, 20)
