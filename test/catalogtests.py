...
class NonCatalogTests(unittest.TestCase):
    def testDimensionInfo(self):
        ...
        def inf(r):
            return DimensionInfo(None, None, None, r, None, None)
...
class CatalogTests(unittest.TestCase):
    ...
    def testStores(self):
        ...
        self.assertEqual(
            "states_shapefile",
            self.cat.get_stores(names="states_shapefile", workspaces=topp.name)[0].name,
        )
        self.assertEqual(
            "states_shapefile", self.cat.get_stores(names="states_shapefile")[0].name
        )
        self.assertEqual(
            "sfdem", self.cat.get_stores(names="sfdem", workspaces=sf.name)[0].name
        )
    ...
    def testResources(self):
        ...
        self.assertEqual(
            2,
            len(
                self.cat.get_resources(
                    stores=[states.name, sfdem.name], workspaces=[topp.name, sf.name]
                )
            ),
        )
        self.assertEqual(
        ...
        self.assertEqual(
            "states",
            self.cat.get_resources(
                names="states", stores=states.name, workspaces=topp.name
            )[0].name,
        )
        self.assertEqual(
            "states",
            self.cat.get_resources(names="states", workspaces=topp.name)[0].name,
        )
        self.assertEqual(
            "states", self.cat.get_resource("states", workspace=topp.name).name
        )
        ...
        self.assertEqual(
            "sfdem",
            self.cat.get_resources(
                names="sfdem", stores=sfdem.name, workspaces=sf.name
            )[0].name,
        )
        self.assertEqual(
            "sfdem", self.cat.get_resources(names="sfdem", workspaces=sf.name)[0].name
        )
    ...
    def testLayers(self):
        ...
        self.assertEqual(
            set(s.name for s in states.styles), set(["pophatch", "polygon"])
        )
    ...
    def testLayerGroups(self):
        ...
        actual = set(
            l.name
            for l in self.cat.get_layergroups(
        ...
        tas2 = self.cat.create_layergroup(
            "tasmania_reloaded", tas.layers, workspace="topp"
        )
        self.cat.save(tas2)
        self.assertEqual(
            1,
            len(self.cat.get_layergroups(names="tasmania_reloaded", workspaces="topp")),
        )
    ...
    def testEscaping(self):
        ...
        self.assertEqual(
            ...
            len(
                self.cat.get_stores(
                    workspaces="best workspace ever", names="best store ever"
                )
            ),
        )
    ...
    def testUnicodeUrl(self):
        ...
        self.assertEqual(
            u,
            f"{self.cat.service_url}/workspaces/test/datastores/operaci%C3%B3n_repo/featuretypes.xml",
        )
        ...
        self.assertEqual(
            u,
            f"{self.cat.service_url}/workspaces/test/datastores/test-repo/featuretypes.xml",
        )
...
class ModifyingTests(unittest.TestCase):
    ...
    def testPublishFeatureType(self):
        ...
        try:
            ...
            self.cat.publish_featuretype(
                "import", ds, native_crs="EPSG:4326", native_name=data_source_name
            )
            # and now it's not
    ...
    def testVirtualTables(self):
        ...
        ft = self.cat.publish_featuretype(
            ft_name, ds, epsg_code, jdbc_virtual_table=jdbc_vt
        )
    ...
    def testCoverageSave(self):
        ...
        self.assertEquals(
            set(rs.request_srs_list), srs_before, str(rs.request_srs_list)
        )
        ...
        self.assertEquals(
            set(rs.response_srs_list), srs_before, str(rs.response_srs_list)
        )
        rs.response_srs_list = rs.response_srs_list + ["EPSG:3785"]
        self.cat.save(rs)
        rs = self.cat.get_resources("Arc_Sample", workspaces="nurc")[0]
        self.assertEquals(
            set(rs.response_srs_list), srs_after, str(rs.response_srs_list)
        )
        ...
        rs = self.cat.get_resources("Arc_Sample", workspaces="nurc")[0]
        self.assertEquals(
            set(rs.supported_formats), formats_after, str(rs.supported_formats)
        )
    ...
    def testWmsLayer(self):
        ...
        for layer in available_layers:
            ...
            new_layer = self.cat.create_wmslayer(
                wmstest, wmsstore, name, nativeName=layer
            )
        ...
        self.assertEqual(
            layer.resource.projection_policy, changed_layer.projection_policy
        )
    ...
    def testFeatureTypeCreate(self):
        ...
        self.assertRaises(
            ...
            lambda: self.cat.create_featurestore(
    ...
    def testWorkspaceStyles(self):
        ...
        self.cat.create_style(
            "jed", open("test/ted.sld").read(), overwrite=True, workspace="topp"
        )
        self.cat._cache.clear()
        jed = self.cat.get_styles(names="jed", workspaces="topp")
        self.assert_(len(jed) == 1)
    ...
    def testLayerWorkspaceStyles(self):
        # upload new style, verify existence
        self.cat.create_style(
            "ned", open("test/fred.sld").read(), overwrite=True, workspace="topp"
        )
        self.cat.create_style(
            "zed", open("test/ted.sld").read(), overwrite=True, workspace="topp"
        )
        self.cat._cache.clear()
    ...
    def testTimeDimension(self):
        ...
        def get_resource():
            return (
                self.cat._cache.clear()
                or self.cat.get_layer("boxes_with_end_date").resource
            )

        # configure time as LIST
        resource = get_resource()
        timeInfo = DimensionInfo(
            "time", "true", "LIST", None, "ISO8601", None, attribute="date"
        )
